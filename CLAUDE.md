...
- **components/** — `App` manages tab state (Services/Monitor/Settings) and form dialog. `Layout` renders header with tab navigation and hostname (with hover tooltip showing OS-specific change instructions). Services tab: `ServiceTable` > `ServiceRow`, `ServiceFormDialog` + `TxtRecordEditor`, `BulkActions`. Monitor tab: `MonitoringView` > `StatusDashboard` + `LogViewer` + `NetworkInfo`. Settings tab: `SettingsView` with config import/export and hostname details.
